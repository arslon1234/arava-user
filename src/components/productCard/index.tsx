      ...
      <div className="w-full h-[45%] flex flex-col justify-between p-1">
        <div>
          <p className="text-[18px] md:text-[24px] font-semibold">
            {price.toLocaleString("fr-FR")} so`m
          </p>
          <p className="text-[16px] md:text-[18px] line-clamp-2 text-gray-600 leading-5 md:leading-6">{title}</p>
        </div>
        <button className="flex items-center gap-2 w-full shadow-[0_0_10px_#00000013] bg-white hover:bg-white/50 duration-200 justify-center h-[35px] md:h-[40px] rounded-xl">
          <span>
            <FaPlus className="text-[18px] md:text-[20px]" />
          </span>
          <span className="text-[16px] md:text-[18px]">Qo`shish</span>
        </button>
      </div>
    </div>
  );
